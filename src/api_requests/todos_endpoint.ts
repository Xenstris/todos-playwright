...
export const todosEndpoint = {
    ...
    getAllTodos: async function (apiContext: APIRequestContext) {

        const getAllTodosResponse = await apiContext.get(endpoint);
        return getAllTodosResponse.json();
    },

    deleteTodo: async function (apiContext: APIRequestContext, id:string) {

        const deleteTodoResponse = await apiContext.delete(endpoint + `/${id}`);
        if (!deleteTodoResponse.ok()) {
            throw new Error(`Error: ${deleteTodoResponse.status()} - task with following id could not be deleted: ${id}`);
        }
    },
